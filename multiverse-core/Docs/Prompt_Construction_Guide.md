# 📝 Prompt Construction Guide

> GM **не интерпретирует**, не оценивает, не задаёт тон. Он **собирает объективные факты** и формирует два сообщения для `/v1/chat/completions`:  
> - `role: system` — контекст,  
> - `role: user` — задача и требования.

---

## 🧱 Структура промта

### `system` message

    Ты — Повествователь Мира. Твоя задача — развивать историю естественно, иммерсивно и поэтично.

    ### ФАКТЫ МИРА
    • "Бессмертие нарушает Закон Сохранения Души"

    ### СУЩНОСТИ В ОБЛАСТИ
    - player:123: 
      Состояние: HP=45, location=forbidden_lab
      Параметры: perception=0.4, fear=0.7

    ### ОКРУЖЕНИЕ
    - Время: 03:17 (ночь)
    - Погода: густой туман, дождь
    - Локация: Запретная лаборатория (руны на стенах)

### `user` message

    ### ЗАДАЧА
    Подумай: что *логично* происходит дальше?
    — Учитывай факты, характеры, обстановку.
    — Если уместно, используй стилевые модификаторы: 
      «внезапно», «трагично», «иронично», «поэтично», «мрачно» и т.д.
    — Ты волен определить тон и атмосферу самостоятельно.

    ### ТРЕБОВАНИЯ
    1. Ответ строго в формате JSON.
    2. "narrative": 1–3 предложения, для игроков.
    3. "mood": массив строк (опционально).
    4. "new_events": массив (макс. 3) событий.

    ### ФОРМАТ ОТВЕТА
    {
      "narrative": "строка",
      "mood": ["тег1", "тег2"],
      "new_events": [ ... ]
    }

---

## 🔍 Как формируются блоки

| Блок | Источник | Пример |
|------|----------|--------|
| `ФАКТЫ МИРА` | `KnowledgeBase.Canon` | `"Бессмертие нарушает Закон..."` |
| `СУЩНОСТИ` | `entity.state`, `entity.parameters` | `"HP=45, perception=0.4"` |
| `ОКРУЖЕНИЕ` | `world.weather`, `location.description` | `"густой туман, дождь"` |
| `СОБЫТИЯ` | `humanizeDuration(dt)` | `"[через мгновение] погас факел"` |

> ✅ Только факты. Никаких оценок.